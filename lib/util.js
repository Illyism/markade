...
function checkConfig(directory) {
  ...
  try {
    ...
    if (err.code && err.code == "MODULE_NOT_FOUND")
      return checkJson(directory, err);
...
function checkJson(directory, prevErr) {
  var deferred = q.defer();
  var filePath = path.resolve(directory, "markade.json");
  fs.readFile(filePath, "utf-8", function (err, res) {
    if (err) {
      error("  %s- %s%s", c.bldred, prevErr.message, c.txtrst);
      error("  %s- %s%s", c.bldred, err.message, c.txtrst);
...
function writefile(fp, contents) {
  ...
  mkdirp(path.dirname(fp), function (err) {
    ...
    fs.writeFile(fp, contents, function(err, res) {
      if (err) {
        error("  %s❌ %s%s", c.bldred, c.txtrst, err);
        deferred.reject(new Error(err));
      } else {
...
function readfile(fp) {
  var deferred = q.defer();
  fs.readFile(fp, "utf-8", function (err, res) {
    if (err) {
      error("  %s❌ %s%s", c.bldred, c.txtrst, err);
...
function mkdir(dir) {
  ...
  fs.mkdir(dir, function (err, res) {
    if (err && err.code != "EEXIST") {
      ...
      log("  %s✔ %s%s", c.bldgrn, c.txtrst, dir);
      deferred.resolve(res);
    }
  });
  return deferred.promise;
...
function copy(from, to) {
  ...
  fs.createReadStream(from)
    ...
    .on("finish", function() {
      log("  %s✔ %s%s", c.bldgrn, c.txtrst, to);
      deferred.resolve(to);
    });
  return deferred.promise;
}
