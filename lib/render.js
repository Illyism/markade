...
function determineTemplate(item, markFile, json) {
  ...
  // **1.** .md data file with a `template: file.pug` defined.
  ...
  var _template = path.basename(item.data, ".md") + ".pug";
    ...
    util.readfile(item.dataPath)
      ...
      .then(function(markFile) {
        return q.fcall(function () {
          ...
          try {
            var pugFn = pug.compileFile(template);
            ...
            if (err.code === "ENOENT") {
              throw new Error("There is no defined pug file for this markdown document");
            } else {
              throw err; // rethrow
            }
          }
...
function singleStart(data, template, options, callback) {
  ...
  render(data)
    .then(function(markFile) {
      ...
      return q.fcall(function () {
        var pugFn = pug.compile(template, options.pug);
        return pugFn({
          $: markFile.normal.meta,
          html: markFile.normal.html,
          blocks: markFile,
